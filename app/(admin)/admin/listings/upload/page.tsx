...
import { Upload, FileSpreadsheet, CheckCircle2, XCircle, ArrowLeft, Filter } from 'lucide-react';
...
export default function ListingUploadPage() {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      fd.append('file', file);
      const res = await fetch('/api/admin/listings/upload', { method: 'POST', body: fd });
      if (!res.ok) throw new Error();
      const json = await res.json();
      setPreview(json.data);
      // 기본으로 전체 선택
      setSelected(new Set(json.data.rows.map((_: FilteredRow, i: number) => i)));
    } catch {
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
      ...
      <div className="flex items-end justify-between border-b border-gray-200 pb-5">
        <div>
          <Link href="/admin/listings" className="inline-flex items-center gap-1 text-xs text-gray-400 hover:text-gray-600 mb-2 transition-colors">
            <ArrowLeft className="w-3 h-3" /> 공고 DB 관리로 돌아가기
          </Link>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-xl border border-gray-100 bg-white px-5 py-4 shadow-sm">
              ...
              <p className="text-xs text-gray-400 mt-1">엑셀 전체 공고</p>
            </div>
            <div className="rounded-xl border border-gray-100 bg-white px-5 py-4 shadow-sm">
              <p className="text-xs text-gray-400 mb-1">파싱 성공</p>
              <p className="text-2xl font-semibold text-gray-900">{preview.parsed}</p>
              <p className="text-xs text-gray-400 mt-1">회사명+직무 인식</p>
            </div>
          ...
          {/* 직군별 분류 */}
          {preview.categoryStats && (
            <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-sm">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-3">직군별 분류</p>
              <div className="grid grid-cols-4 gap-3">
                {Object.entries(preview.categoryStats).map(([cat, count]) => (
          ...
          {preview.rows.length === 0 ? (
            <div className="rounded-xl border border-gray-100 bg-white py-16 text-center shadow-sm">
              ...
              <p className="text-sm text-gray-400">컴퓨터공학부 관련 직군이 없습니다</p>
              <p className="text-xs text-gray-300 mt-1">개발, IT, 데이터, 사무직 등의 키워드가 포함된 공고만 필터링됩니다</p>
            </div>
          ) : (
            <>
              {/* 테이블 */}
              <div className="rounded-xl border border-gray-100 bg-white shadow-sm overflow-hidden">
                <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-gray-50/50">
                  ...
                  <span className="text-xs text-gray-400">총 {preview.rows.length}개 필터링됨</span>
                </div>
                        ...
                        >
                          ...
                          <td className="px-4 py-3 hidden md:table-cell">
                            <div className="space-y-0.5">
                              {row.career && <p className="text-xs text-gray-500">{row.career}</p>}
                              {row.employType && <p className="text-xs text-gray-400">{row.employType}</p>}
      ...
      <div className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
        ...
        <p className="text-xs text-gray-400 mt-3">* 필수 항목. 컬럼명이 다르더라도 유사한 이름이면 자동 인식됩니다.</p>
      </div>
    </div>
  );
}
